...
export default function ProjectCard({ image, name, description, url }: any) {
  return (
    <Link href={process.env.NEXT_PUBLIC_GITHUB_PERSONAL + url} target="blank">
      <div className="flex flex-col bg-bgCard rounded-xl border-2 border-borderCard shadow-2xl overflow-clip h-80 md:max-w-sm">
        <Image src={image} alt={name} className="h-3/5 object-contain" />
